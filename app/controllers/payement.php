...
$pay =  $this->load_model('Pay');
$pay -> order_save($data['rows'],$data['user_data'],$data['sub_total'] );
// foreach($data['rows'] as $row){
//      $pay -> order_save($row->book_,$data['user_data']);}

        // show($data['user_data']);

    //  show($_SESSION);
        $data['page_title'] = "Payement";
        $this->view("payement",$data);
    }
}